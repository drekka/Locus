...
public class Setting<T> {
    ...
    public init(wrappedValue _: T, _ key: String, container: @escaping () -> SettingsContainer = { SettingsContainer.shared }) {
        self.key = key
        self.container = container
        log.info("🧩 Setting: Default values cannot be set using the property wrapper. Defaults should be set during registration or loaded from a default value source.")
    }
